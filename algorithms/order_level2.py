    ...
    for cut2 in level2_sorted:
        ...
        if cut2['nItem'] != 0: #ACA SOLO ACTUALIZARA LA PRIMERA PIEZA, SI HAY PIEZAS IGUALES EN EL MISMO TABLERO, ESTAS TIENEN DISTINTAS CORRDENADAS E IGUAL NUMERO DE ITEM, debe validarse tambien por antigua coordenada
          part = next((part for part in parts 
                   if part['nItem'] == cut2['nItem'] 
                   and round(part['x'] + (part['width'] if part['rotated'] is False else part['length']) + saw_width/2, 2) == round(old_x2, 2)
                   and round(part['y'] + (part['length'] if part['rotated'] is False else part['width']) + saw_width/2, 2) == round(cut2['y2'], 2)
                   )
                  , None)
          if part:
...
def sort_level2_by_strip_width(nested_structure, parts, trim, saw_width, order='asc'):
    ...
    for cutLvl1 in nested_structure:
        if(cutLvl1['level2']):
            ...
            level2_sorted, parts = recalculate_all_coordinates(level2_sorted, parts, trim, saw_width)
